...
describe('Diffy commands — end-to-end through real QuickPick UI', () => {
  ...
  it('compareWith → SideB=pickRef → user picks the v0.1.0 tag → diff against that ref', async () => {
    ...
    // RefPicker: alphabetical — refs/heads/main first, refs/tags/v0.1.0 second
    // Pick the v0.1.0 tag (item 2) → moveNext × 1
    await moveAndAccept(1);
    ...
    assert.match(
      ...
      new RegExp(`^${shas.third.slice(0, 7)} ↔ ${shas.second.slice(0, 7)} — `),
    );
